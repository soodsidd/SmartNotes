...
1. **Desktop ribbon vs. floating toolbar** — Should the desktop editor use a fixed formatting ribbon (OneNote-style, always visible above editor) or a floating toolbar that appears on text selection? Ribbon is more discoverable; floating is less cluttered.

2. **Section groups** — Does the left tree need section groups in Phase 1, or can that wait until Phase 3? Adds IA complexity early.

3. **Bottom tab labels and icons (phone)** — Notes, Capture, Search, AI. What icons work at 24px without labels? Or should labels always show?

4. **Notebook color system** — Currently 7 predefined colors. Custom colors? Colors used as visual identifiers in tree and page list?

5. **AI panel conversation history** — Multi-turn within a session (resets on page close), or persistent per-page history stored in vault metadata?

6. **Annotation entry point** — Floating "Annotate" button on page view, or a mode toggle at the top? On tablet, annotation mode replaces the editor pane — is that the right model or should they be side-by-side?

7. **Capture on desktop** — Modal dialog (Ctrl+Shift+V) or a panel that slides in from the side?
